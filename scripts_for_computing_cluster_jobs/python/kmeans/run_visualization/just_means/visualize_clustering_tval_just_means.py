...
def get_template_to_pid_to_cond_to_matrix(light_id, in_dir = "/scratch/gpfs/KNORMAN/rkempner/clustering_output/"):
...
def get_template_to_pid_to_cond_to_matrix_with_json(light_id, in_json_dir = "/scratch/gpfs/KNORMAN/rkempner/clustering_output/"):
    template_to_pid_to_cond_to_matrices = {}
        ...
        for template_id in cluster_to_template_to_cond_to_event_to_violin[cluster_id]:
            for cond in cluster_to_template_to_cond_to_event_to_violin[cluster_id][template_id]:
                    for event in cluster_to_template_to_cond_to_event_to_violin[cluster_id][template_id][cond]:
                        ...
                        for this_event_mean_this_participant, pid in zip (info_dict["mean_each_pid"], info_dict["pids_list"]):
...
def process_cluster(return_dict, cluster_label, df, condition_types = 
    ...
    for template_id in [2,3,4]:
        ...
        for cond in condition_types:
            ...
            for event in [2,3,4]:
                ...
                temp_dict[template_id][cond][event] = {"mean_each_pid":this_event_mean_each_pid, "pids_list": unique_pids_list}
...
def from_cluster_labels_to_fingerprint_avg_df(df):
    manager = Manager()
...
K = job_id_in
cluster_assignment_csv_path = "/scratch/gpfs/KNORMAN/rkempner/clustering_output/kmeans_assignments_just_means/kmeans_" + str(K) + "clusters_just_means.csv"
assignment_df = pd.read_csv(cluster_assignment_csv_path)
in_json_dir = "/scratch/gpfs/KNORMAN/rkempner/clustering_output/"
raw_df = from_cluster_labels_to_fingerprint_avg_df(assignment_df)
out_path = "/scratch/gpfs/KNORMAN/rkempner/clustering_output/kmeans_fingerprints_just_means/kmeans_" + str(K) + "clusters_just_means" 
out_path += ".csv"
