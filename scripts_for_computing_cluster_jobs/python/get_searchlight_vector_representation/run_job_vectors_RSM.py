...
def get_template_event(held_out_event_name, held_out_event_id, template_id):
    pass

def construct_RSM_from_480_files(in_dir, subset_list_of_searchlights):
    os.chdir(in_dir)
    RSM_matrix_before_pid_mean = np.zeros(shape = (11,11,40))
    for counter_light, light_id in enumerate(subset_list_of_searchlights):
        ...
        for file_name in files_this_light:
            ...
            try:
                ...
                pdb.set_trace()
            for wedding_id in range(new_arr.shape[0]):
                this_wedding_row = new_arr[wedding_id,:]
                this_wedding_event2 = this_wedding_row[event_2_start:event_3_start]
