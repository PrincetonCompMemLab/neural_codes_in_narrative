...
def kmeans_clustering(dir, K):
    ...
    X = np.load(file = dir + "master_X_list_just_means.npy")
    searchlight_list = np.load(file = dir + "master_searchlight_just_means.npy").tolist()
    print("K_clusters: ", K)
    ...
    df.to_csv(dir + "kmeans_" + str(K) + "clusters_just_means.csv")
    # It is important to use binary access
    with open(dir + "kmeans_" + str(K) + "clusters_just_means.pickle", 'wb') as f:
        pickle.dump(kmeans, f)
  

job_id_in = int(os.environ["SLURM_ARRAY_TASK_ID"])
dir = "/scratch/gpfs/KNORMAN/rkempner/clustering_output/kmeans_assignments_just_means/"
K = job_id_in
