...
# The same-event/same-schema condition is always the correct-path condition, and the other-event/same-schema condition is always the other-path-from-same-schema condition.
    if (len(corr_list)) != (36*40):
        print("Error: (len(corr_list)) != (36*40)")
        return
    output_dict = {
...
def process_cluster(return_dict, cluster_label, df, cluster_id_to_distilled_lights, condition_types = 
    ...
    print("all: ",len(df[df["cluster_assignment"] == cluster_label]["searchlight"].tolist()))
    print("fraction: ", len(cluster_id_to_distilled_lights[cluster_label]))
...
distill_it = True
by_sil = True
distill_numLights = 0.25
fraction = True
optional_focus = None
if distill_it:  
    cluster_id_to_distilled_lights = get_cluster_id_to_distilled_searchlights(K, distill_numLights, exaggerated_0, weight_it, cap_zero, fraction, by_sil)
else:
...
if distill_it:
    if by_sil:
        out_path += "_filtered_by_silhouettes"

    out_path += ("_top" + str(distill_numLights*100) + "percent")
out_path += ".csv"
